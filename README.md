...
SELECT DISTINCT COUNT(*) FROM РАБОТА
JOIN "МЕСТО РАБОТЫ" ON РАБОТА."МЕСТО РАБОТЫ" = "МЕСТО РАБОТЫ".ИДЕНТИФИКАТОР
where "МЕСТО РАБОТЫ"."НАЗВАНИЕ ОРГ." = 'Университет' and РАБОТА.ДАТА IN ('Январь', 'Февраль', 'Март');

![image](https://user-images.githubusercontent.com/64357780/115125409-4a6c4680-9fe1-11eb-843d-8b5154e39116.png)

d)	какие работники получали плату выше средней.

SELECT DISTINCT СОТРУДНИК.ФАМИЛИЯ FROM СОТРУДНИК
JOIN "РАБОТА" ON РАБОТА."СОТРУДНИК" = "СОТРУДНИК".ИДЕНТИФИКАТОР
where РАБОТА.ПЛАТА > (SELECT AVG(РАБОТА.ПЛАТА) FROM РАБОТА);

![image](https://user-images.githubusercontent.com/64357780/115125540-3bd25f00-9fe2-11eb-9bfe-e4ec1161cfee.png)
...
SELECT СОТРУДНИК.ФАМИЛИЯ, MAX(ПЛАТА) FROM СОТРУДНИК
JOIN "РАБОТА" ON РАБОТА."СОТРУДНИК" = "СОТРУДНИК".ИДЕНТИФИКАТОР
GROUP BY СОТРУДНИК.ФАМИЛИЯ;

![image](https://user-images.githubusercontent.com/64357780/115125832-37a74100-9fe4-11eb-9f2e-5682f0c07e1c.png)
